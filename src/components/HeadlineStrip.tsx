...
export default function HeadlineStrip({ year }: HeadlineStripProps) {
  ...
  return (
    <div className="grid grid-cols-4 gap-px bg-[#35b779]/[0.08] border-b border-[#35b779]/[0.15]">
      {items.map(({ icon: Icon, iconColor, value, label, sub }) => (
        <div
          key={label}
          className="flex items-center gap-3 px-4 py-2.5 bg-background hover:bg-[#35b779]/[0.04] transition-colors"
        >
          <div className="w-7 h-7 rounded-lg bg-[#35b779]/[0.07] flex items-center justify-center shrink-0">
            <Icon className={`w-4 h-4 ${iconColor}`} />
          </div>
          <div>
            <div className="text-base font-bold font-mono text-[#111827] leading-none">{value}</div>
            <div className="text-xs text-[#374151] mt-0.5 leading-none">{label}</div>
            <div className="text-[11px] text-[#9ca3af] leading-none mt-0.5">{sub}</div>
          </div>
        </div>
      ))}
    </div>
