...
export default function SoilProfile({ soil, source, bare = false }: Props) {
  if (!soil) {
    if (bare) return <p className="text-[11px] text-gray-500">Soil data unavailable.</p>;
    return (
      <div className="rounded-xl p-3 border border-white/[0.06] bg-white/[0.02]">
        ...
        <p className="text-[11px] text-gray-500">Soil data unavailable.</p>
      </div>
    );
  }
  ...
  const phColor =
    ...
    soil.pH < 5.5 ? 'text-yellow-400' :
    soil.pH <= 7.0 ? 'text-emerald-400' :
    soil.pH <= 7.5 ? 'text-yellow-400' : 'text-orange-400';
  ...
  const body = (
    <>
      <div className="rounded-md bg-white/[0.03] border border-white/[0.04] px-2.5 py-2 mb-2">
        <div className="flex items-center gap-1.5 mb-0.5">
          <Layers className="w-3 h-3 text-amber-400" />
          <span className="text-xs font-bold text-white">{soil.type}</span>
          <span className="text-[10px] text-gray-500">({soil.textureClass})</span>
        </div>
        <p className="text-[11px] text-gray-500 leading-snug">{soil.description}</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-1.5 mb-2">
        ...
        />
        <Metric
          icon={<Sprout className="w-3 h-3 text-green-400" />}
          label="Organic C"
      ...
      <div className="mb-2">
        <div className="flex items-baseline justify-between mb-1">
          <span className="text-[10px] text-gray-500 uppercase tracking-wider">Texture</span>
          <span className="text-[10px] text-gray-600 font-mono">
            {sand.toFixed(0)} / {silt.toFixed(0)} / {clay.toFixed(0)}
          </span>
        </div>
        <div className="flex h-3 rounded-full overflow-hidden border border-white/[0.06]" aria-label={`Sand ${sand.toFixed(0)}%, silt ${silt.toFixed(0)}%, clay ${clay.toFixed(0)}%`}>
          <div className="bg-amber-300/70" style={{ width: `${sand}%` }} title={`Sand ${sand.toFixed(0)}%`} />
          <div className="bg-stone-400/70" style={{ width: `${silt}%` }} title={`Silt ${silt.toFixed(0)}%`} />
          <div className="bg-orange-700/70" style={{ width: `${clay}%` }} title={`Clay ${clay.toFixed(0)}%`} />
        </div>
      ...
      <div className="text-[10px] text-gray-500 mt-1 text-right">cm³/cm³</div>
    </>
  );
...
function Mini({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className={`rounded-md px-2 py-1.5 border ${highlight ? 'bg-accent/[0.06] border-accent/20' : 'bg-white/[0.03] border-white/[0.04]'}`}>
      <div className="text-[10px] text-gray-500 uppercase tracking-wider">{label}</div>
      <div className={`text-xs font-bold font-mono ${highlight ? 'text-accent' : 'text-white'}`}>{value}</div>
    </div>
  );
}
