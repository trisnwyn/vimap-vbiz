...
export default function AIAnalysisPanel({ year, selectedProvince, density = 'compact' }: AIAnalysisPanelProps) {
  ...
  const [aiSource, setAiSource] = useState<'groq' | 'server-fallback' | 'local' | null>(null);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (res.status === 429) {
        ...
        setRateLimitMsg(`Too many requests — please wait ${secs}s before re-analyzing.`);
        // Fall through to local analysis so the panel isn't left empty
      } else if (res.ok) {
        ...
        if (data.insights && data.insights.length > 0) {
          ...
          setInsights(mapped);
          setAiSource(data.model === 'rule-based-fallback' ? 'server-fallback' : 'groq');
          setAnalyzing(false);
          return;
        }
      }
    ...
    <div className="p-2 rounded-md bg-[#35b779]/[0.04] border border-[#35b779]/[0.12] animate-fade-in">
      <div className="flex items-center gap-1.5 mb-1">
        <div className={`w-1.5 h-1.5 rounded-full ${
          aiSource === 'groq' ? 'bg-accent' : aiSource === 'server-fallback' ? 'bg-amber-400' : 'bg-[#9ca3af]'
        }`} />
        <span className="text-[11px] font-bold text-[#374151]">
          {aiSource === 'groq'
            ? 'Powered by Groq (Llama 3.3 70B)'
            : aiSource === 'server-fallback'
            ? 'Rule-based analysis (AI service unavailable)'
      ...
      <p className="text-[11px] text-[#6b7280] leading-relaxed">
        {aiSource === 'groq'
          ...
          : aiSource === 'server-fallback'
          ? 'AI service is temporarily unavailable. Showing data-driven insights computed server-side from province statistics. Set GROQ_API_KEY in .env.local for full AI analysis.'
  ...
  const rateLimitBanner = rateLimitMsg && (
    <div className="flex items-start gap-1.5 px-2.5 py-1.5 rounded-md bg-amber-500/[0.08] border border-amber-500/20 animate-fade-in">
      <AlertTriangle className="w-3 h-3 text-amber-500 mt-0.5 shrink-0" />
      <p className="text-[11px] text-[#374151] leading-snug">{rateLimitMsg} Showing local analysis below.</p>
    </div>
  );
